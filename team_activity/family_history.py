...
def print_death_age(people_dict):
    ...
    for key, value in people_dict.items():
        ...
        print(
            f"{name} {age_at_death} | {value[BIRTH_YEAR_INDEX]} - {value[DEATH_YEAR_INDEX]}")
...
def print_marriages(marriages_dict, people_dict):
    ...
    for key, items in marriages_dict.items():
        husband_id = items[HUSBAND_KEY_INDEX]
        wife_id = items[WIFE_KEY_INDEX]
        husband = people_dict[husband_id]
        wife = people_dict[wife_id]

        print(f"{husband[NAME_INDEX]} {items[WEDDING_YEAR_INDEX] - husband[BIRTH_YEAR_INDEX]} >\
 {items[WEDDING_YEAR_INDEX]} < {wife[NAME_INDEX]} {items[WEDDING_YEAR_INDEX] - wife[BIRTH_YEAR_INDEX]}")


def count_marriages(marriages_dict, people_dict):
    print("Number of Marriages")

    for key, info in people_dict.items():
        info.append(0)
    
    for key, info in marriages_dict.items():
        husband_id = info[HUSBAND_KEY_INDEX]
        wife_id = info[WIFE_KEY_INDEX]
        husband = people_dict[husband_id]
        husband[4] += 1
        wife = people_dict[wife_id]
        wife[4] += 1
    ...
    for key, list in people_dict.items():
        ...
        number_marriages = list[4]
        print(name, number_marriages)
